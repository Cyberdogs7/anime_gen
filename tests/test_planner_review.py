...
def test_dialogue_pass_is_written_for_the_ear():
    """The DIALOGUE WRITER pass must carry the spoken-word contract: short,
    conversational lines that become synthesized audio, delivery for tone,
    voice anchors per character."""
    from studio.planner import _SCENE_PASSES
    from studio.prompts import scene_pass_prompt

    dlg = next(p for p in _SCENE_PASSES if p["name"] == "dialogue")
    assert "2-20 words" in dlg["instructions"]
    assert "SYNTHESIZED AUDIO" in dlg["instructions"]
    assert "voice-anchor" in dlg["instructions"]
    assert "`delivery`" in dlg["instructions"]
    assert "SPOKEN, not read" in dlg["instructions"]

    p = scene_pass_prompt(dlg, {"title": "Demo"}, "ep summary", {"id": "s01"},
                          [{"name": "Ryou", "personality": ["quiet"],
                            "traits_for_llm": "terse, dry"}],
                          [{"id": "s01_sh01"}], 110)
    assert "SPOKEN-AUDIO CONTEXT" in p
    assert "2-20 words" in p
    assert "H3 lip-sync" in p
...
def test_reconciler_regenerates_rejected_plan_from_notes(tmp_path, monkeypatch):
    ...
    (Auto mode: the plan gate is off, so the reconciler regenerates instead of
    waiting for a human.)"""
    ...
    monkeypatch.setattr("studio.episode_repair._plan_auto", lambda *a, **k: True)
...
def test_plan_gate_holds_pending_outline_for_human(tmp_path, monkeypatch):
    """With gates.plan=gated (the default), a passing outline stays pending for
    human approval — the reconciler must NOT auto-approve it."""
    from studio.episode_repair import _advance_episode, _plan_auto

    assert _plan_auto() is False                 # plan gate is gated by default

    show = _make_show(tmp_path)
    runs = show.dir / "runs" / "EP01"
    runs.mkdir(parents=True, exist_ok=True)
    (runs / "plan.json").write_text(json.dumps({
        "episode": 1, "status": "pending", "plot": "good",
        "plan_review": {"rounds": 1, "passed": True, "notes": []}}), encoding="utf-8")
    ...
    _advance_episode(show, 1)
    plan = json.loads((runs / "plan.json").read_text(encoding="utf-8"))
    assert plan["status"] == "pending"          # held for human outline approval


def test_reconciler_regenerates_rejected_plan_even_when_gated(tmp_path, monkeypatch):
    """A REJECTED outline (with notes) must be regenerated by the reconciler even
    when the plan gate is gated — rejection is an instruction to rewrite, not a
    wait-for-human state. This is the bug that stranded EP01 as 'rejected' with
    nothing picking it back up."""
    from studio.episode_repair import _advance_episode, _plan_auto

    assert _plan_auto() is False                 # gated
    ...
    (runs / "plan.json").write_text(json.dumps({
        "episode": 1, "status": "rejected",
        "rejected_notes": "does not cover how the girls find blade",
        "plot": "old broken plot"}), encoding="utf-8")
    ...
    _advance_episode(show, 1)
    plan = json.loads((runs / "plan.json").read_text(encoding="utf-8"))
    assert captured["notes"] == "does not cover how the girls find blade"
    assert plan["plot"] == "rewritten"
    # The regenerated outline goes back to the HUMAN gate (plan is gated) — it is
    # not stranded as 'rejected', and not silently auto-approved either.
